...
const Dashboard = () => {
  ...
  const previousTotalContactsCount = useMemo(() => {
  ...
  const previousNewContactsCount = useMemo(() => {
          ...
          <CardContent>
            ...
            {selectedPeriod !== "all" && (
              <p className="text-xs flex items-center">
                ...
                <span className={cn("ml-1", getTrendTextColor(totalContactsCount, previousTotalContactsCount))}>
                  {previousTotalContactsCount}
                </span>
                {getTrendIcon(totalContactsCount, previousTotalContactsCount)}
              </p>
            )}
            {selectedPeriod === "all" && (
        ...
        <Card className="w-full">
          ...
          <CardContent>
            ...
            {selectedPeriod !== "all" && (
              <p className="text-xs flex items-center">
                <span className="text-foreground">{getPreviousPeriodLabel(selectedPeriod)}:</span>
                <span className={cn("ml-1", getTrendTextColor(newContactsCount, previousNewContactsCount))}>
                  {previousNewContactsCount}
                </span>
                {getTrendIcon(newContactsCount, previousNewContactsCount)}
              </p>
            )}
            {selectedPeriod === "all" && (
              <p className="text-xs text-muted-foreground">
