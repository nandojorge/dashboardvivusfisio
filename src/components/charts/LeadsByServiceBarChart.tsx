...
interface LeadsByServiceBarChartProps {
  currentLeads: Contact[];
  selectedPeriod: FilterPeriod;
  dateRange: { from?: Date; to?: Date };
}

const LeadsByServiceBarChart: React.FC<LeadsByServiceBarChartProps> = ({
  currentLeads,
  selectedPeriod,
  dateRange,
}) => {
  ...
  const data = useMemo(() => {
    ...
    currentLeads.forEach(lead => {
      const service = lead.servico || 'Desconhecido';
      counts[service] = (counts[service] || 0) + 1;
    });

    const chartData = Object.keys(counts).map(service => ({
      name: service,
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="rounded-lg border bg-card p-2 shadow-sm">
          ...
          <p className="text-xs text-muted-foreground">{entry.value} Leads</p>
        </div>
      );
    }
    return null;
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        ...
        <CardDescription>Distribuição de leads por serviço para {getPeriodLabel(selectedPeriod)}</CardDescription>
      </CardHeader>
