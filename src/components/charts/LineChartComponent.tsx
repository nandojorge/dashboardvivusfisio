...
const LineChartComponent: React.FC<LineChartComponentProps> = ({
    ...
    if (period === "custom" && range?.from && range?.to) {
      ...
      switch (period) {
        ...
        case "year":
          ...
          break;
        case "all":
          const allDates = currentContacts.map(c => c.dataContacto && typeof c.dataContacto === 'object' ? c.dataContacto.getTime() : Infinity)
  ...
  const chartData = useMemo(() => {
    ...
    const filteredItems = currentContacts.filter(contact => {
      const contactDate = contact.dataContacto instanceof Date ? contact.dataContacto : parseISO(contact.dataContacto as string);
      return isWithinInterval(contactDate, { start: intervalStart, end: intervalEnd });
    });
    ...
    filteredItems.forEach(item => {
      const itemDate = item.dataContacto instanceof Date ? item.dataContacto : parseISO(item.dataContacto as string);
      let key: string;
      if (spanInDays <= 60) {
    ...
    return sortedData;
  }, [currentContacts, selectedPeriod, dateRange]);
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        <CardTitle>Tendência de Contactos</CardTitle>
